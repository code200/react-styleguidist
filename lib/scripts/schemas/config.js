...
process.platform === 'win32' ? /* istanbul ignore next: no windows on our test plan */`src/components/**/*.{${EXTENSIONS}}` : `src/@(components|Components)/**/*.{${EXTENSIONS}}`;
...
const configSchema = {
  ...
  getExampleFilename: {
    ...
    default: componentPath => {
      const files = [_path.default.join(_path.default.dirname(componentPath), 'Readme.md'),
      // ComponentName.md
      componentPath.replace(_path.default.extname(componentPath), '.md'),
...
var _default = exports.default = configSchema;