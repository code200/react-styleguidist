...
const getSectionComponents = (section, config) => {
  ...
  const componentFiles = (0, _getComponentFiles.default)(section.components, config.configDir, ignore);
  ...
  const uniqueFiles = componentFiles.filter((file, index, arr) => {
    ...
    return arr.findIndex(f => _path.default.dirname(f) === dir) === index;
  });
  return (0, _getComponents.default)(uniqueFiles, config);
};
