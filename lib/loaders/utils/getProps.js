...
const JS_DOC_ALL_SYNONYMS = [...JS_DOC_METHOD_PARAM_TAG_SYNONYMS, ...JS_DOC_METHOD_RETURN_TAG_SYNONYMS];
...
const getDocletsObject = str => ({
  ..._reactDocgen.utils.docblock.getDoclets(str)
});
...
function getProps(doc, filepath) {
  ...
  outDocs.methods = outDocs.methods.map(method => {
    ...
    const params = method.params && method.params.map(param => ({
      ...param,
  ...
  if (doc.props) {
    ...
    Object.keys(doc.props).forEach(propName => {
      ...
      const documentation = _doctrine.default.parse(prop.description || '');
